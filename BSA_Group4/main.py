        ...
        def login(self, master):
            ...
            def viewbutton():
                ...
                try:
                    ...
                    messagebox.showerror("Error","Invalid to date")
...
class CustomerFrame(Frame):
    def __init__(self, master=None):
        ...
        def selectBook():
            ...
            ls=BookDB().get_bookbyisbn(tempisbn)
            if ls.get_noofcopies() == 0:
                r =messagebox.showinfo("Not enough books!","Enter Details for Procurement")
                if r=="ok":
                    newwin = Toplevel()
                    frame = Procurement(newwin)
            else:
        ...
        def login(self, master):
            ...
            def add():
                ...
                for x in tranid:
                    ...
                    if str(x[1])==isbn_:
                        print(100)
                        dummy=TransactionDB().set_paid(True,x[0],int(copies_),int(isbn_))
                        if dummy==1:
                            ...
                            def savePub( master, publishername):
                                ...
                                if not len(vendors_):
                                    ...
                                    def saveven( master, publishername):
                                        ...
                                        c.execute("SELECT * FROM Vendors WHERE vendorName=?",[vendor_name])
                                        vendors_=c.fetchall()
                                        conn.commit()
                                        print(vendors_)
                                    ...
                                    savevenbutton = Button(master, text="Save Vendor", font=("Helvetica", 10),
                                                           width=20, height=1, bg='green',
