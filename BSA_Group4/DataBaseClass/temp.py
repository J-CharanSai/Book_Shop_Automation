...
c.execute(" INSERT INTO Employees VALUES (?,?,?,?,?)",[13, 'jerry', 'jerr', 'tom', 2])
...
conn.commit()
c.execute(" INSERT INTO books  VALUES (?,?,?,?,?,?,?,?,?)",['x-men','stanlee',800,87000,1,2,2,10,10])
conn.commit()
...
conn.commit()
c.execute("UPDATE Transactions SET paid=False WHERE transactionId = 10")
conn.commit()
c.execute(" INSERT INTO books  Vendors ('Relativity the special and generan theory','albert einstein',180,97881,1,1,2,10,10)")
'''
